...
// 🚀 NEW DASHBOARD SKELETON COMPONENT
const DashboardSkeleton = () => (
  <div className="space-y-8 pb-10 h-full flex flex-col w-full">
    ...
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative z-20">
      <div>
        <div className="h-9 w-72 bg-zinc-800/60 rounded-lg animate-pulse mb-3"></div>
        <div className="h-4 w-96 bg-zinc-800/40 rounded-md animate-pulse"></div>
      </div>
      ...
      {[1, 2, 3, 4].map((i) => (
        ...
        >
          <div className="flex justify-between items-start mb-4">
            ...
            <div className="h-7 w-24 rounded-lg bg-zinc-800/50"></div>
          </div>
          <div className="mt-6">
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 p-6 md:p-8 rounded-2xl bg-white/[0.02] border border-white/5 h-[450px] animate-pulse">
          <div className="flex justify-between items-center mb-6">
            <div className="h-8 w-48 bg-zinc-800/60 rounded-lg"></div>
            <div className="h-7 w-24 bg-zinc-800/50 rounded-lg"></div>
          </div>
          <div className="w-full h-[320px] bg-zinc-800/30 rounded-xl"></div>
        </div>

        <div className="p-6 md:p-8 rounded-2xl bg-white/[0.02] border border-white/5 h-[450px] animate-pulse flex flex-col">
          <div className="flex justify-between items-center mb-6">
      ...
      <div className="w-full p-6 md:p-8 rounded-2xl bg-white/[0.02] border border-white/5 h-[450px] animate-pulse">
        <div className="flex justify-between items-center mb-6">
          <div className="h-8 w-56 bg-zinc-800/60 rounded-lg"></div>
          <div className="h-7 w-24 bg-zinc-800/50 rounded-lg"></div>
        </div>
        <div className="w-full h-[320px] bg-zinc-800/30 rounded-xl"></div>
      </div>
...
const Dashboard = () => {
    ...
    datasets: [
      {
        ...
        borderColor: isTransport ? "#0ea5e9" : "#3b82f6",
        backgroundColor: isTransport
  ...
  const productionChartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        titleColor: isTransport ? "#22d3ee" : "#a78bfa",
        bodyColor: "#f4f4f5",
  ...
  const blueChartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        backgroundColor: "rgba(9, 9, 11, 0.95)",
        titleColor: isTransport ? "#38bdf8" : "#60a5fa",
        bodyColor: "#f4f4f5",
        borderColor: "rgba(39, 39, 42, 1)",
        borderWidth: 1,
        padding: 12,
...
const StatCard = ({
  ...
  return (
    <div
      ...
      className={`bg-white/[0.02] backdrop-blur-md border border-white/5 p-6 rounded-2xl ${hoverClass} cursor-pointer transition-all duration-300 group hover:-translate-y-1 relative overflow-hidden shadow-2xl shadow-black/20`}
    >
...
const ActivityItem = ({ data, isTransport, basePath }) => {
  ...
  if (data.activityType === "Sale") {
    ...
    title = `Sale: ${data.buyerName || "Customer"}`;
    desc = data.productName || "Items Sold";
    amount = `₹${Number(data.amount || 0).toLocaleString("en-IN")}`;
    linkUrl = `${basePath}/sales/report?highlight=${data._id}`;
    ...
    desc = `Billed to ${data.client?.name || "Client"}`;
    amount = `₹${Number(data.grandTotal || 0).toLocaleString("en-IN")}`;
    linkUrl = `${basePath}/invoices?highlight=${data._id}`;
  }

  return (
    <div
      onClick={() => navigate(linkUrl)}
      className="group flex items-start gap-4 p-3 rounded-xl hover:bg-white/[0.04] cursor-pointer transition-all border border-transparent hover:border-white/10 active:scale-[0.98]"
    >
      <div
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-start">
          ...
          {amount && (
            <p className="text-xs font-bold text-zinc-300 font-mono ml-2 group-hover:text-white transition-colors">
              {amount}
            </p>
          )}
        </div>
