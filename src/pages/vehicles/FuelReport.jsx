...
const FuelReport = () => {
  ...
  const filteredLogs = useMemo(() => {
    return logs.filter((log) => {
      ...
      const matchSearch = String(log.vehicleNo || "")
        .toLowerCase()
        .includes(searchTerm);

      let matchAmount = true;
      const amt = Number(log.totalCost) || 0;
      if (filters.amountFilter === "Under ₹5k") matchAmount = amt < 5000;
      else if (filters.amountFilter === "₹5k - ₹20k")
        matchAmount = amt >= 5000 && amt <= 20000;
      else if (filters.amountFilter === "Over ₹20k") matchAmount = amt > 20000;
    ...
    try {
      ...
      const rows = logs.map((log) => {
        let dateStr = log.date
          ...
          : "-";
        return `${dateStr},"${log.vehicleNo}",${log.liters},${log.pricePerLiter},${log.totalCost}`;
      });
      ...
      link.setAttribute(
        "download",
        `Full_Fuel_Backup_${new Date().toISOString().split("T")[0]}.csv`,
      );
  ...
  const handleExport = () => {
    try {
      ...
      link.setAttribute(
        "download",
        `Fuel_Report_${new Date().toISOString().split("T")[0]}.csv`,
      );
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
  ...
  const handleWipeAll = async () => {
    ...
    setWiping(true);
    try {
      await fuelService.deleteAllLogs({ password: deletePassword });
